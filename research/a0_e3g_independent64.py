#!/usr/bin/env python3
"""Frozen 64-seed independent validation for Adaptive CFG followed by E3-G."""
...
RESULT = ROOT / "results/a0_e3g_independent64"
REPORT = ROOT / "reports/a0_e3g_independent64"
LOG = ROOT / "logs/a0_e3g_independent64"
EXTERNAL_TOOLS = ROOT / "tools/a0_e3g_independent64"
...
BRANCH = "feature/a0-e3g-independent64"
...
def write_master(stage: str, status: str, **values: Any) -> None:
    payload = (
        ...
        if MASTER_PROGRESS.is_file()
            ...
            "schema_version": 1,
            "experiment": "A0 + E3-G independent validation 64",
...
def configure_modules() -> None:
    ...
    core.relax_rows = independent_relax_rows
...
def refine() -> None:
    ...
    features["method"] = "A0"
    features["split"] = "a0_e3g_independent64_independent"
...
def independent_relax_rows() -> list[dict[str, Any]]:
...
def analyze() -> None:
    ...
    state = (
        "A0_E3G_INDEPENDENT64_GO"
        if primary["primary_effect_pass"] and safety_pass
        else "A0_E3G_INDEPENDENT64_NO_GO"
    )
    ...
    atomic_text(
        ...
        "# Reproduce A0 + E3-G independent validation 64\n\n```bash\n"
        "source /data/dxl/env.sh\n"
        "cd /data/dxl/mattergen_v1\n"
        "git switch feature/a0-e3g-independent64\n"
        "/data/dxl/tools/a0_e3g_independent64/resume.sh\n"
        "/data/dxl/tools/a0_e3g_independent64/status.sh\n"
        "```\n",
    )
    atomic_text(
    ...
    write_master(
        "independent_validation_decision",
        "success",
        final_state=state,
        a0_e3g_independent64_go=state == "A0_E3G_INDEPENDENT64_GO",
...
def validate_frozen_state() -> dict[str, Any]:
    if not subprocess.run(
        ...
        raise RuntimeError("independent validation branch does not descend from E3-G formal")
    frozen_a0_paths = (
    ...
    if changed_a0_paths:
        raise RuntimeError(
            f"A0 implementation changed after independent validation base: {changed_a0_paths}"
        )
    expected_assets = {
...
def initialize() -> None:
    ...
    manifest = {
        ...
        "e3g_formal_commit": E3G_FORMAL_COMMIT,
        "independent_base_commit": BASE_COMMIT,
        "independent_branch": BRANCH,
    ...
    atomic_text(
        ...
        "# A0 + E3-G independent validation freeze\n\n"
    ...
    write_master("seed_audit", "success", evaluation_seeds=[50000, 50063])
...
def pipeline() -> None:
    ...
    with PIPELINE_LOCK.open("a+") as lock:
        try:
            ...
            raise RuntimeError("another independent validation pipeline is running") from error
        initialize()
        for command in (
            [str(MATTERGEN_PYTHON), "-m", "research.a0_e3g_independent64", "generate"],
            [str(CHGNET_PYTHON), "-m", "research.a0_e3g_independent64", "refine"],
            [str(MATTERGEN_PYTHON), "-m", "research.a0_e3g_independent64", "relax"],
            [str(MATTERGEN_PYTHON), "-m", "research.a0_e3g_independent64", "analyze"],
        ):
