"""Run 128 CrystalREPA reproduction relaxations with persistent MatterSim workers."""
...
def generation_dir(method: str, seed: int) -> Path:
    return GENERATION / method / f"seed_{seed}"
...
def freeze_config() -> dict:
    ...
    source.update(
        {
            "experiment": "CrystalREPA unconditional MP-20 U0 versus R1",
...
def launch() -> int:
    ...
    set_stage(
        "sixty_four_relax",
        "running",
        "Running 128 MatterSim-5M relaxations with 16 persistent workers.",
        {"workers_per_gpu": 2, "total_concurrency": 16, "tasks": 128},
    )
    existing = core.relax_initialize()
    existing_valid = [
        task
        for task in existing["tasks"]
        if task["status"] == "success" and core.validate_relax(task)
    ]
    if len(existing_valid) == 128:
        converged = sum(bool(task["converged"]) for task in existing_valid)
        summary = {
            "created_at": now(),
            "success": 128,
            "converged": converged,
            "valid_but_not_converged": 128 - converged,
            "workers_per_gpu": 2,
            "total_concurrency": 16,
            "checkpoint_sha256": MATTERSIM_SHA,
            "skipped_resume": True,
        ...
        set_stage(
            "sixty_four_relax",
            "success",
            f"Resume audit reused 128/128 finite relax outputs; converged={converged}.",
            summary,
        )
        return 0
    processes = []
    ...
    for gpu in range(8):
        for slot in range(2):
            env = core.task_environment(gpu, f"crystalrepa_relax_gpu{gpu}_slot{slot}")
            env["MATTERGEN_CRYSTALREPA_RELAX_WORKER"] = "1"
            ...
            process = subprocess.Popen(
                [
                    str(PYTHON),
                    "-m",
                    "research.crystalrepa_repro.run_mattersim_relaxation",
                    "relax-worker",
                    "--gpu",
                    str(gpu),
                    "--slot",
    ...
    if len(valid) != 128 or any(code not in (0,) for code in return_codes):
        set_stage(
            "sixty_four_relax",
            "failed",
            f"MatterSim relaxation incomplete: valid={len(valid)}/128.",
    ...
    set_stage(
        "sixty_four_relax",
        "success",
        f"128/128 finite relax outputs; converged={converged}, valid-at-cap={128-converged}.",
        summary,
    )
    return 0
