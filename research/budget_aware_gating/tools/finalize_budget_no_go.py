...
def sha(p):
 ...
 return h.hexdigest()
summary=read(SCREEN/'quality_speed_summary.json')['configs']; decisions=read(SCREEN/'candidate_decisions.json')['decisions']; old=read(REPORT/'frozen_formal_baseline/innovation2_g3_config.json')['decision']; smoke=read(REPORT/'eight_seed_go_no_go.json')
rows=[]
...
gs=old['speed']; gq=old['quality']['G3']; rows.append({'config':'G3','experiment_stage':'old_formal_independent_seeds','paired_with_A0':False,'screen_decision':'reference_only','speed_multiplier':1+gs['single_worker_normalized_median_speedup'],'throughput_multiplier':1+gs['fixed_concurrency_throughput_gain'],'physical_forward_reduction':gs['physical_forward_reduction'],'corrector_skip_rate':old['mechanism']['corrector_skip_rate'],'avg_energy_above_hull_per_atom':gq['avg_energy_above_hull_per_atom'],'frac_stable_structures':gq['frac_stable_structures'],'frac_novel_unique_stable_structures':gq['frac_novel_unique_stable_structures'],'warning':'Different old formal seed set; excluded from new paired statistics.'})
