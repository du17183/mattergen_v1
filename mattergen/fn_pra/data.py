...
class RepaCrystalDataset(Dataset):
    """CrystalDataset wrapper that attaches strictly mapped atom-wise teacher rows."""
    ...
    def __init__(self, base: CrystalDataset, teacher_cache_path: str | Path) -> None:
        ...
        self.features = np.load(
            self.teacher_cache_path / "features.f16.npy",
            mmap_mode="r",
        )
        self.teacher_atomic_numbers = np.load(
            self.teacher_cache_path / "atomic_numbers.npy",
            mmap_mode="r",
        )
        ...
        if len(base) != len(self.structure_ids):
            raise ValueError(
                f"Dataset/cache structure count mismatch: {len(base)} != {len(self.structure_ids)}"
            )
        if not np.array_equal(np.asarray(base.structure_id, dtype=str), self.structure_ids.astype(str)):
    ...
    def __getitem__(self, index: int):
        ...
        cached_atomic_numbers = np.asarray(self.teacher_atomic_numbers[start:stop], dtype=np.int64)
        graph_atomic_numbers = graph["atomic_numbers"].detach().cpu().numpy().astype(np.int64)
        ...
        teacher_features = torch.from_numpy(np.asarray(self.features[start:stop], dtype=np.float16).copy())
