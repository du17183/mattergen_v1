"""Compute formal metrics from shared reference and precomputed GPU relaxations."""
...
def main() -> None:
    memory_fd = os.memfd_create("global_transformer_p0_reference", os.MFD_ALLOW_SEALING)
    ...
    try:
        ...
        if failures:
            raise RuntimeError(f"formal metric failures: {failures}")
    finally:
        os.close(memory_fd)
