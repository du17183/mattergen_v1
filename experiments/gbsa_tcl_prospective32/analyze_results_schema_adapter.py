...
def load_method(method: str, generation: pd.DataFrame) -> dict[str, np.ndarray]:
    ...
    generated_structures = [AseAtomsAdaptor.get_structure(item) for item in generated_atoms]
    comp_validity = [is_smact_valid(structure) for structure in generated_structures]
    struct_validity = [structure_validity(structure) for structure in generated_structures]
    data = {
        ...
        "composition_validity": np.asarray(comp_validity, dtype=float),
        "structure_validity": np.asarray(struct_validity, dtype=float),
