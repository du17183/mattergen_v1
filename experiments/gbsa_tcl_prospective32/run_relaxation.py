"""Run frozen MatterSim-5M relaxation for both prospective arms on GPUs."""
...
def main() -> None:
    ...
    running = []
    for gpu, method in zip(GPU_IDS, METHODS):
        ...
        print(json.dumps({"event": "relaxation_started", "method": method,
                          "gpu": gpu, "pid": process.pid}), flush=True)
    failures = []
    for method, process, stream, log_path in running:
        return_code = process.wait()
        stream.close()
        print(json.dumps({"event": "relaxation_complete", "method": method,
                          "return_code": return_code}), flush=True)
        if return_code:
