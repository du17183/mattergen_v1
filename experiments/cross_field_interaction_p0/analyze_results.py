"""Create the compact CFI P0 quality table and print seed-level tails."""
...
def main() -> None:
    ...
    for method in METHODS:
        ...
        initial = read(
            ROOT / "relaxation" / method / "initial_with_properties.extxyz",
            index=":",
        )
        force_norms = [
            np.linalg.norm(structure.get_forces(), axis=1)
            for structure in initial
        ]
        all_force_norms = np.concatenate(force_norms)
        structure_force_means = np.asarray(
            [values.mean() for values in force_norms]
        )
        structure_force_max = np.asarray(
            [values.max() for values in force_norms]
        )
        ...
        print(json.dumps({
            "method": method,
            "seeds": list(SEEDS),
            "formulas": generated["formula"].tolist(),
            "e_hull_by_seed": dict(zip(map(str, SEEDS), e_hull.tolist())),
            "rmsd_by_seed": dict(zip(map(str, SEEDS), rmsd.tolist())),
            "structure_max_force_by_seed": dict(
                zip(map(str, SEEDS), structure_force_max.tolist())
            ),
            "relaxation_steps_by_seed": dict(
                zip(map(str, SEEDS), relaxation_steps.tolist())
            ),
        }, indent=2), flush=True)
