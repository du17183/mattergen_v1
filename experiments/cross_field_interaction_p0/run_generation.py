...
CHECKPOINTS = {
    "MLP": PREVIOUS_ROOT / "checkpoints/MLP/adapter.pt",
    "CFI": ROOT / "checkpoints/CFI/adapter.pt",
}
...
def preflight() -> dict[str, str]:
    if SEEDS != tuple(range(77000, 77008)):
        raise RuntimeError("CFI P0 must use exactly seeds 77000-77007")
    if not (MODEL_ROOT / "checkpoints/last.ckpt").is_file():
        raise FileNotFoundError(MODEL_ROOT / "checkpoints/last.ckpt")
    training = {
        row["method"]: row
        for row in csv.DictReader((ROOT / "training_summary.csv").open())
    }
    hashes = {}
