"""Generate one frozen 1000-step MatterGen CFI P0 sample."""
...
def main() -> None:
    ...
    if (args.method == "C0") != (args.adapter_checkpoint is None):
        raise ValueError("C0 takes no adapter; MLP/CFI require their checkpoint")
    ...
    run_dir = (
        ...
        / "generation" / args.method / str(args.seed)
    )
    ...
    if args.adapter_checkpoint is not None:
        ...
        if checkpoint.get("architecture") != args.method:
            raise ValueError(
                f"adapter mismatch: requested {args.method}, checkpoint has "
        ...
        if args.method == "MLP":
            ...
            generator.model.diffusion_module.model.gemnet.global_adapter = adapter
        else:
            adapter = CrossFieldAdapter().to(device).eval()
            adapter.load_state_dict(checkpoint["adapter_state_dict"], strict=True)
            generator.model.diffusion_module.model.gemnet.cross_field_adapter = adapter
        adapter_parameter_l2 = float(torch.sqrt(sum(
