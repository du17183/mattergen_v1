"""Generate one real 1000-step Predictor-Corrector sample."""
...
def main() -> None:
    ...
    if (args.method == "C0") != (args.adapter_checkpoint is None):
        raise ValueError("C0 takes no adapter checkpoint; adapters require one")
    ...
    if args.adapter_checkpoint is not None:
        ...
        expected_metadata = "M2" if args.method == "M2-DB" else args.method
        if checkpoint.get("method") != expected_metadata:
            raise ValueError(
                f"adapter method mismatch: {args.method} expects metadata "
                f"{expected_metadata}, got {checkpoint.get('method')}"
            )
