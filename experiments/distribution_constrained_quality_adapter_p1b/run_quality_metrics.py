"""Compute official MatterGen metrics for C0/M1/M2 with one shared reference."""
...
ROOT = PROJECT_ROOT / "experiments/distribution_constrained_quality_adapter_p1b/screen16"
...
def main() -> None:
    memory_fd = os.memfd_create("quality_adapter_p1b_screen16_reference", os.MFD_ALLOW_SEALING)
    ...
    try:
        for method in METHODS:
            ...
            command = [
                ...
                method,
                "--structures-path",
                str(ROOT / "structures" / f"{method}_generated.extxyz"),
                "--potential-path",
                str(POTENTIAL),
                "--reference-memory-fd",
                str(memory_fd),
                ...
                str(output),
                "--temporary-dir",
                str(ROOT / "runtime_tmp"),
                "--device",
                "cpu",
                "--precomputed-relaxation-dir",
                str(ROOT / "relaxation" / method),
            ]
