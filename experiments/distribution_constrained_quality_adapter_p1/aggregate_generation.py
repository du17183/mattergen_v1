"""Materialize seed-ordered P1 structures for MatterSim evaluation."""
...
def main() -> None:
    ...
    for method in METHODS:
        ...
        for seed in SEEDS:
            ...
            if int(summary["sampling_steps"]) != 1000 or float(summary["guidance_scale"]) != 2.0:
                raise ValueError(f"sampling config mismatch for {method} seed {seed}")
        ...
        write(output, atoms)
        print(json.dumps({"method": method, "count": len(atoms), "seed_min": min(SEEDS), "seed_max": max(SEEDS)}))
