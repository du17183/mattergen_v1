# 第3章证据包：MatterGen基线、数据与评价体系

> 本文件是写作证据，不是完整论文正文。任何项目事实必须回指 source_id；未支持内容不得由通用知识补齐。

## 1. 本章研究目标

定义条件晶体生成任务、C0基线、数据资格、代理评价和配对统计口径。
...
承接理论章节，为第4章Adaptive CFG、第5章E3-PCR及第6章组合/审计提供统一基线和评价口径。
...
先建立证据等级，再介绍指标；所有Stable/E-hull/RMSD旁保留surrogate限定。不要把现有旧提纲的章节编号自动带入本证据包。
